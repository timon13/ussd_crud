...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.ussd.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ussds.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="ussd_code">{{ trans('cruds.ussd.fields.ussd_code') }}</label>
                <input class="form-control {{ $errors->has('ussd_code') ? 'is-invalid' : '' }}" type="text" name="ussd_code" id="ussd_code" value="{{ old('ussd_code', '') }}" required>
