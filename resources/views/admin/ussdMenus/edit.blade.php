...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.ussdMenu.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.ussd-menus.update", [$ussdMenu->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ussd') ? 'is-invalid' : '' }}" name="ussd_id" id="ussd_id">
                    @foreach($ussds as $id => $entry)
                        <option value="{{ $id }}" {{ (old('ussd_id') ? old('ussd_id') : $ussdMenu->ussd->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('ussd'))
